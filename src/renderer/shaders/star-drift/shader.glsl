...
void main() {
  ...
  for (int i = 0; i < 3; i++) {
    ...
    vec2 q = p * grid + vec2(t * (0.12 + layer * 0.05), t * 0.035) + layer * 17.3;
    vec2 cell = floor(q);
    vec2 seed = hash(cell);
    vec2 offset = fract(q) - (0.25 + seed * 0.5);
    float distance = length(offset);
    float radius = mix(0.018, 0.045, seed.x) * (1.0 - layer * 0.18);
    ...
    float halo = exp(-distance * distance / 0.008) * 0.12;
    ...
    float pulse = 1.0 - twinkle * (0.5 + 0.5 * sin(t * 0.5 + seed.y * 6.283185));
    light += mix(color1, color2, seed.x) * glow * pulse * (0.9 - layer * 0.2);
  }
  vec3 background = vec3(0.004, 0.007, 0.02) + color1 * 0.012
    * (0.5 + 0.5 * sin(p.x * 2.0 + p.y * 3.0 + t * 0.04));
  gl_FragColor = vec4(1.0 - exp(-(background + light) * brightness), 1.0);
}
